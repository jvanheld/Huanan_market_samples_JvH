...
parameters$dirs$data <- file.path(parameters$dirs$base, "data")
...
## Input files
infiles <- list()
infiles$mito_composition_per_sample <- file.path(parameters$dirs$data, "mito_composition_by_sample.csv")
...
```{r samples_per_species, fig.width=12, fig.height=8, out.width="100%", fig.cap="Number of positive samples per species (top) and of different species per sample (bottom)"}
...
```{r samples_vs_species_heatmap, fig.width=12, fig.height=12, out.width="100%", fig.cap="Heatmap showing the number of aligned reads per sample (rows) and species common name (columns). Counts were log10-converted to highlight the full range of values. Zero counts are displayed in yellow. "}
...
## Focus on the raccoon dog

```{r racoon_dog, fig.width=10, fig.height=8, out.width="80%", fig.cap="Number of reads (log10-transformed) aligned against Racoon dog (abcsissa) versus SARS-CoV-2 (ordinate)."}

selected_species <- "raccoon dog"

#### Focus on samples containing DNA from a given species ####
species_subset <- subset(mito_composition_per_sample, common_name == selected_species)
nrow(species_subset)
plot(species_subset$aligned_reads_log10, species_subset$SARS2_aligned_reads_log10,
     xlab = paste0("Aligned on mitochondrial genome from ", selected_species, ", log10(counts)"),
...
nozero_subset <- subset(species_subset, species_subset$aligned_reads*species_subset$SARS2_aligned_reads > 0)
nrow(nozero_subset)
...
Bloom JD. Association between SARS-CoV-2 and metagenomic content of samples from the Huanan Seafood Market. Microbiology, 2023 DOI:10.1101/2023.04.25.538336.

Liu WJ, Liu P, Lei W, et al. Surveillance of SARS-CoV-2 at the Huanan Seafood Market. Nature 2023; published online April 5. DOI:10.1038/s41586-023-06043-2.
